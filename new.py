...
def _run_webcam_inference():
    ...
    while True:
        ...
        if vec is not None:
            ...
            if probability < 0.80:
                prediction_text = f"Low conf ({probability:.2f})"
            else:
                prediction_text = f"Pred: {pred} ({probability:.2f})"

            # Show the preprocessed 28x28 image for debugging
            processed = (vec.reshape(28, 28) * 255).astype(np.uint8)
            proc_vis = cv2.resize(processed, (280, 280), interpolation=cv2.INTER_NEAREST)
            cv2.imshow('Preprocessed (28x28)', proc_vis)
